...
1. Locate `veomni/models/transformers/<M>/`. If the directory does not exist yet
   you are being called as the modeling step of `/veomni-model-migration` in
   new-model mode: create it, and read that skill's
   `references/new-model-integration.md` first so the category (text / VLM / Omni,
...
- The rest of onboarding a new model — deciding the model category, the
  ...
  `/veomni-model-migration` in new-model mode, which hands the modeling step back here.
